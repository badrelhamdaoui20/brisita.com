if (typeof SALESBOX_API == "undefined") {

    var SALESBOX_API = 'https://salesboxapi.fireapps.io/api/';
    var SALESBOX_STATS_API = 'https://salesboxstats.fireapps.io/';
    var GET_IP_SERVER = 'https://api.ipify.org/?format=json';
    ...
    function Salesbox($) {
        ...
        this.addHtml = function (setting, html) {
            ...
            if (setting.position_type == 'custom_element') {
                ...
                var length = $('form[action$="/cart/add"]').length;
                if (length > 0) {
                    ...
                    if (select_element.length == 0) {
                        select_element = form_select_element.find('input').last();
                    }
                }
